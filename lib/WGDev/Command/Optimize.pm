...
sub optimise_assets {
    ...
    for my $asset ( sort keys %{ $session->config->get('assets') } ) {
        push @assets, $asset
            unless $session->db->quickScalar('select count(*) from asset where className = ?', [$asset]);
    }
    
    if (@assets) {
        $self->report("The following Assets do not appear in your Asset table:\n");
        $self->report("\t$_\n") for @assets;
...
sub optimise_macros {
    ...
    for my $macro ( sort keys %{ $session->config->get('macros') } ) {
        push @macros, $macro
            unless $session->db->quickScalar("select count(*) from template where template like ? or template like ?", ["%^$macro;%", "%^$macro(%"]);
    }
    
    if (@macros) {
        $self->report("The following Macros do not appear in the template field of the template table:\n");
        $self->report("\t$_\n") for @macros;
        my $config = $wgd->config_file();
        my $message = <<"END_MESSAGE";
